...
OUTDIR=/fast/users/a1662801/20190520_TigerSnake_RNASeq/2_alignedData/bam
...
f1=${DATADIR}/464A_FRANKLINISLAND.trimmed1.fq.gz
f2=${DATADIR}/464A_FRANKLINISLAND.trimmed2.fq.gz
echo "Read 1 is "$f1" "
echo "Read 2 is "$f2" "


# Align trimmed reads against reference using STAR
echo "Starting alignment of "$(basename ${f1%.trimmed1.fq.gz})" "
    
STAR --genomeDir /data/biorefs/reference_genomes/animals/Notechis_scutatus/star \
     --runThreadN 8 \
     --readFilesCommand gunzip -c \
     --readFilesIn ${f} ${f2} \
     --outFileNamePrefix ${OUTDIR}/$(basename ${f1%.fq.gz}) \
     --outSAMtype BAM SortedByCoordinate

echo "Finishing alignment of "$(basename ${f1%.trimmed1.fq.gz})" "
