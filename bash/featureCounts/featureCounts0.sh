...
featureCounts -T 8 \
	...
	-g gene_id \
	-a ${GTF} \
	-o ${OUTDIR}/counts0.txt \
	${DATADIR}/*out.bam

## Create matrix for downstream DE analyses
cut -f 1,7-52 ${OUTDIR}/counts0.txt > ${OUTDIR}/counts0Matrix.txt

# Clean up matrix for downstream DE analyses
# Delete first line of txt file & clean up sample names
sed -i '1d;$d' ${OUTDIR}/counts0Matrix.txt
sed -i '1s:/fast/users/a1662801/20190520_TigerSnake_RNASeq/2_alignedData/bam3/:S:g'${OUTDIR}/counts0Matrix.txt
sed -i '1s:Aligned.sortedByCoord.out.bam::g' ${OUTDIR}/counts0Matrix.txt
